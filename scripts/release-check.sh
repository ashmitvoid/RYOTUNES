...
grep -q 'version = "2.4.0"' Cargo.toml || { say FAIL 'workspace version is not 2.4.0'; fail=1; }
grep -q '"version": "2.4.0"' src-tauri/tauri.conf.json || { say FAIL 'Tauri version is not 2.4.0'; fail=1; }
grep -q '"version": "2.4.0"' ui/package.json || { say FAIL 'UI version is not 2.4.0'; fail=1; }
