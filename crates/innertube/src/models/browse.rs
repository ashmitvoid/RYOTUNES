//! Browse-surface parsing: home feed, library playlists, playlist/album pages. browse parser.
//!
...
use super::metadata::{
    ...
    is_video_endpoint, is_video_row, last_thumbnail, list_item_video_id, parse_list_item, play_count,
    runs_text, runs_text_opt,
    ArtistRun, SongItem,
};
...
pub struct BrowseItem {
    ...
    /// Song cards only: this card links a video upload, not the generated audio track.
    /// Audio-only endpoints filter these cards before they reach the app.
    #[serde(default)]
    pub is_video: bool,
    /// Song cards only: one of the signed-in user's own uploads.
    #[serde(default)]
    pub is_upload: bool,
...
pub struct Section {
    pub title: String,
    /// The carousel header itself links to an artist channel. YouTube sometimes uses that raw
    /// artist name for a mixed recommendation shelf; the UI normalizes those to a generic
    /// recommendation heading instead of implying every card belongs to the seed artist.
    #[serde(default)]
    pub title_is_artist: bool,
...
/// The orders YouTube can put a playlist in. browse parser.
///
/// YouTube stores this choice itself, which is why Ryotunes asks for a sorted page instead of
...
pub enum PlaylistSort {
    ...
    Artist,
    Album,
    /// YouTube's "Top voted". Ryotunes's own menu does not offer it, but a list can already be in
...
/// The sort menu in a playlist header, when YouTube offers one. browse parser.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct SortMenu {
...
pub struct PlaylistPage {
    ...
    pub owned: bool,
    /// Collaboration is enabled for this playlist.
...
/// An artist detail page (`browse` on a `UC…` channel id). browse parser.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
/// header fields (artist link, type/year, description). browse parser.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
/// Parse a `FEmusic_home` response into filter chips + titled carousel sections. browse parser.
...
pub fn parse_home(root: &Value) -> HomePage {
    ...
    for shelf in find_all(root, "musicCarouselShelfRenderer") {
            ...
            .is_some_and(|browse| {
                browse
                    ...
                    || browse
                        ...
                        .and_then(|cfg| cfg.get("browseEndpointContextMusicConfig"))
                        .and_then(|cfg| cfg.get("pageType"))
                        .and_then(Value::as_str)
        ...
        if !items.is_empty() {
            ...
            sections.push(Section { title, title_is_artist, items, more_browse_id, more_params });
        }
    }
    HomePage { chips, sections, continuation: continuation_token(root) }
}

/// Parse a `FEmusic_liked_*` response into a flat grid of cards. browse parser. Playlists and albums
...
/// Parse a playlist/album (`VL…` / `MPRE…`) browse response. browse parser.
...
/// Parse a browse continuation response (more playlist tracks). browse parser.
...
/// shelves YouTube returns (songs / albums / artists / playlists). browse parser.
...
/// One filtered search page. YouTube returns a bounded first page and an opaque continuation;
/// carrying that token through the native boundary lets the UI incrementally extend search without
/// speculative requests or an artificial fixed result ceiling.
#[derive(Debug, Clone, Default, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct SearchCardPage {
    pub items: Vec<BrowseItem>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub continuation: Option<String>,
}
...
/// per-type shelves — so we classify each row by its own navigation target. browse parser.
pub fn parse_search_all(root: &Value) -> SearchResults {
    ...
    r.continuation = continuation_token(root);
    r
}
...
pub fn parse_search_cards(root: &Value) -> Vec<BrowseItem> {
    parse_search_cards_page(root).items
}

/// Parse one filtered search page plus its next continuation.
pub fn parse_search_cards_page(root: &Value) -> SearchCardPage {
    SearchCardPage {
        items: find_all(root, "musicResponsiveListItemRenderer")
            .into_iter()
            .filter_map(list_item_to_browse_item)
            .collect(),
...
/// Parse an album (`MPRE…`) browse response. browse parser.
...
/// fallback (the artist browseId is itself the channelId). browse parser.
...
/// (`VL…`, `PL…`, `RD…`) is treated as a playlist. browse parser.
...
/// they open the normal artist page like every other artist card. browse parser.
...
/// Paging token, modern (`continuationCommand.token`) or legacy (`nextContinuationData`). browse parser.
...
mod tests {
    ...
    #[test]
    fn collaborative_facepile_is_detected() {
        let page = |facepile: Value| {
            json!({ "contents": { "sectionListRenderer": { "contents": [{
                "musicResponsiveHeaderRenderer": { "facepile": facepile }
            }] } } })
        };
        let owner_only = json!({ "avatarStackViewModel": { "avatars": [{ "avatarViewModel": {} }] } });
