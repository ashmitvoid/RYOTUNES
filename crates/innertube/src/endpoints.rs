//! High-level endpoint facade over the transport. player model, 08.
...
/// Search filter params (opaque base64). browse parser.
...
impl InnerTube {
    /// `/player` for one client. player model, stream selection.
    ///
    /// `sts` — signature timestamp from the deciphering player.js (cipher runtime); sent as
    /// `playbackContext.contentPlaybackContext.signatureTimestamp` so ciphered clients return
    /// usable formats. `po_token` — the session/streaming PoToken (PoToken flow); sent as
    ...
    /// Raw `search` POST. `params` = a filter (None = the mixed, unfiltered search). browse parser.
    async fn search_raw(
        &self,
        client: &YouTubeClient,
        query: &str,
    ...
    /// Continue a search with YouTube's opaque token. Search continuations use the same query
    /// carrier shape as browse continuations, but stay on the `search` endpoint. The body remains
    /// context-only so the original query/filter state is entirely owned by the token.
    async fn search_continuation_raw(
        &self,
        client: &YouTubeClient,
        token: &str,
    ) -> Result<serde_json::Value, Error> {
        ...
        let path = format!("search?ctoken={enc}&continuation={enc}&type=next");
        self.post(&path, client, &body, true).await
    }

    // --- audio-only product policy ---------------------------------------------------------------
    // Filter video rows at the fetch boundary so every consumer inherits one policy. Classification
    // comes from YouTube metadata (`metadata::is_video_row`), never from title matching.
    ...
    /// Search songs only (`FILTER_SONG`). browse parser.
    ...
    /// Next page of a songs-only search.
    pub async fn search_songs_continuation(
        &self,
        metadata_client: &YouTubeClient,
        token: &str,
    ) -> Result<SearchResult, Error> {
        let value = self.search_continuation_raw(metadata_client, token).await?;
        let mut r = metadata::parse_search(&value);
        self.drop_video_songs(&mut r.items);
        Ok(r)
    }
    ...
    /// Next page of the mixed/unfiltered search.
    pub async fn search_all_continuation(
        &self,
        client: &YouTubeClient,
        token: &str,
    ) -> Result<SearchResults, Error> {
        let value = self.search_continuation_raw(client, token).await?;
        let mut r = browse::parse_search_all(&value);
        self.drop_video_cards(&mut r.top);
        self.drop_video_cards(&mut r.songs);
        Ok(r)
    ...
    pub async fn search_cards(
        ...
        let mut page = browse::parse_search_cards_page(&value);
        self.drop_video_cards(&mut page.items);
        Ok(page.items)
    }

    /// Filtered search with its continuation token, for bounded incremental UI loading.
    pub async fn search_cards_page(
        &self,
        client: &YouTubeClient,
        query: &str,
        category: &str,
    ) -> Result<browse::SearchCardPage, Error> {
        let filter = match category {
            "albums" => FILTER_ALBUM,
            "artists" => FILTER_ARTIST,
            "playlists" => FILTER_COMMUNITY_PLAYLIST,
    ...
    /// Continue a filtered card search. The opaque token already remembers the category.
    pub async fn search_cards_continuation(
        &self,
        client: &YouTubeClient,
        token: &str,
    ) -> Result<browse::SearchCardPage, Error> {
    ...
    /// Up-next queue / radio for a video. browse parser. Uses the metadata client.
    ///
    ...
    pub async fn next(
        ...
        // The seed itself survives if already playing; generated continuation rows remain audio-only.
        next.items.retain(|i| !i.is_video || Some(i.video_id.as_str()) == video_id);
        Ok(next)
    }

    /// Logged-in account summary (`account/account_menu`, write API). Requires a cookie. Also the
    /// source of `dataSyncId` (account identity) and a login-bound visitorData (authentication flow).
    ...
    /// Raw `browse` call (write API, browse parser). `browse_id`/`params` optional; response is the
    ...
    async fn library_grid(
        ...
        // Note: page cap, so a token that never resolves can't spin forever. Raise it if
    ...
    /// response — pass it to get that chip's filtered feed. browse parser.
    pub async fn home(
        &self,
        client: &YouTubeClient,
        params: Option<&str>,
    ...
    /// Library playlists grid (`FEmusic_liked_playlists`). browse parser. Needs login.
    ...
    /// Saved albums grid (`FEmusic_liked_albums`). browse parser. Needs login.
    ...
    /// live under `FEmusic_library_corpus_artists`). browse parser. Needs login.
    ...
    /// A playlist or album page by browseId (`VL…` / `MPRE…`). browse parser.
    ///
    ...
    /// An album page by album browseId (`MPRE…`). browse parser.
    ///
        ...
        if video.contains(&true) {
            if let Some(pl) = &page.playlist_id {
                match self.playlist(client, &format!("VL{pl}"), None).await {
                    // Note: positional match, guarded on equal track counts — the OLAK
    ...
    /// An artist page by channel browseId (`UC…`). browse parser.
    pub async fn artist(
        &self,
        client: &YouTubeClient,
        browse_id: &str,
    ...
    /// via a carousel's "More" button). browse parser.
    ...
    /// Next page of playlist tracks via a continuation token. browse parser.
    ...
    // --- lyrics (browse parser §lyrics; browseId comes from `next`) -----------------------------
    ...
    // --- write actions (write API ✎, authentication flow D7). All auth-gated (SAPISIDHASH). ---------

    /// Rate a video, or clear its rating. write API. The three states are mutually exclusive on
    ...
    /// Add a video to a playlist. write API `browse/edit_playlist`.
    ///
    ...
    /// list the same way. write API `browse/edit_playlist`.
    ///
    ...
    /// Edit the details of a playlist you own. write API `browse/edit_playlist`.
    ///
    ...
    /// that comes back. Two of the three calls are not InnerTube endpoints at all. write API
    ...
    /// write API `browse/edit_playlist`.
    ...
    /// Create a private playlist; returns the new playlistId. write API `playlist/create`.
    ...
    /// Delete a playlist you own. write API `playlist/delete`.
    ...
    /// Subscribe / unsubscribe to a channel (artist). write API `subscription/*`.
...
/// resumable uploader, sitting on `music.youtube.com` under its own path. write API.
...
/// Playlist edit/delete want the raw playlistId; browse gives it `VL`-prefixed. write API.
