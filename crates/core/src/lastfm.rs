...
//!
//! Everything is best-effort (fail-soft policy fail-soft, same as Discord/media): a failed scrobble is
...
//! Note: no offline queue; add one if scrobbles visibly go missing.
//!
...
//! Note: position-based threshold; track real played-time if cheating ever matters.
...
const API_KEY: &str = match option_env!("RYOTUNES_LASTFM_API_KEY") {
    Some(v) => v,
    None => "",
};
const API_SECRET: &str = match option_env!("RYOTUNES_LASTFM_API_SECRET") {
    Some(v) => v,
    None => "",
};
...
impl Scrobbler {
    ...
    async fn now_playing(&self) {
        ...
        match call("track.updateNowPlaying", params, true).await {
            Ok(_) => tracing::debug!("last.fm now playing sent"),
    ...
    async fn scrobble(&self) {
        ...
        match call("track.scrobble", params, true).await {
            Ok(_) => tracing::info!("scrobbled to last.fm"),
